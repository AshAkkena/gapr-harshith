class CohoDownPolicy < ApplicationPolicy
  def create?
  ...
  class Scope < Scope
    def resolve
      if user.has_role?(:technician) or user.has_role?(:state_coordinator) or user.has_role?(:vicar)
        ...
        #scope.all
        scope.where(cohort_id: Cohort.where(provider_id: Provider.with_role(:site_admin, user).or(Provider.with_role(:facilitator, user)).pluck(:id).uniq))
